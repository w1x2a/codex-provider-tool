...
2. 添加供应商，输入 Base URL、Model、API Key 等信息。已有官方身份使用 `responses`，保留“需要 OpenAI auth”。可只保存供应商专属 Key，稍后切换；需要立刻应用时再勾选立即启用。
...
本工具的模型下拉框与 Codex 桌面菜单是两套列表。自 v1.0.8 起，工具只读取中转列表并修改顶层 `model`，不自动维护桌面模型目录或 `model_catalog_json`。重启后 Codex 仍不显示模型时，应单独排查客户端模型目录与任务设置；不要修改聊天数据来补菜单。
